...
- **签名伪装**：Hook 系统 PackageManager，返回自定义签名信息
- **路径重定向**：支持应用资源文件路径重定向（通过注入so进行 Native Hook）
- **跨进程通信**：使用 [KV Storage](https://github.com/xihan123/kv-storage) 实现模块与宿主应用的数据共享
...
- 不生效，需要多次重启模块和宿主(强行停止)，以达到数据同步的目的
